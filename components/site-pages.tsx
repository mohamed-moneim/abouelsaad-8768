...
const fetcher = (path: string) => fetch(`${apiBase}${path}`).then((response) => { if (!response.ok) throw new Error('Portfolio service unavailable'); return response.json() as Promise<PortfolioProject[]> })
